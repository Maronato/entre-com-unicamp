...
export const getAuthCookieOptions = () => ({ maxAge: MAX_AGE, path: "/", sameSite: true, httpOnly: true, secure: process.env.NODE_ENV === "production" })

interface BaseTokenPayload {
  user: Pick<ResourceOwner, "email" | "id">
}
type TokenPayload = BaseTokenPayload & Required<Pick<JWTPayload, "iss" | "aud" | "iat" | "exp">>

async function signAuthToken(resourceOwner: ResourceOwner) {
  const payload: BaseTokenPayload = { user: { id: resourceOwner.id.toString(), email: resourceOwner.email } }
...
async function verifyAuthToken(token: string) {
  ...
  try {
    const result = await jwtVerify(token, jwks, { algorithms: [ALGORITHM], issuer: ISSUER, audience: ISSUER, typ: "JWT" })
...
export function isAuthenticated(req: Pick<NextApiRequest, "cookies" | "headers">) {
  let token: string | undefined = (req.headers.authorization || "").split("Bearer ")[1]
  if (!token) {
...
export async function login(res: NextApiResponse, resourceOwner: ResourceOwner) {
