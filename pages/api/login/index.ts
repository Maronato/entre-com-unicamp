...
export default async function handler(
  ...
  const emailCode = await prisma.email_codes.findFirst({ where: { AND: { code, email } } })
  if (!emailCode) {
    await delay(3000)
    return res.status(401).send("Invalid credentials")
  }
  let resourceOwner = await prisma.resource_owners.findFirst({ where: { email } })
  if (!resourceOwner) {
    resourceOwner = await prisma.resource_owners.create({ data: { email } })
  }
  const user = new ResourceOwner(resourceOwner.id.toString(), resourceOwner.email)
